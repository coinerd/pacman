...
describe('WarpTunnel - handleTunnelWrap (Legacy)', () => {
    describe('Left tunnel wrap', () => {
        test('wraps entity from left edge to right edge', () => {
            ...
            expect(entity.x).toBe(mazeWidth + gameConfig.tileSize);
        });

        test('wraps entity at -tileSize boundary', () => {
            const mazeWidth = gameConfig.mazeWidth * gameConfig.tileSize;
            const entity = {
                x: -gameConfig.tileSize
            };
        ...
        test('wraps entity beyond -tileSize', () => {
            const mazeWidth = gameConfig.mazeWidth * gameConfig.tileSize;
            const entity = {
    ...
    describe('Right tunnel wrap', () => {
        test('wraps entity from right edge to left edge', () => {
            const mazeWidth = gameConfig.mazeWidth * gameConfig.tileSize;
            const entity = {
                x: mazeWidth + gameConfig.tileSize
            };

            handleTunnelWrap(entity);

            expect(entity.x).toBe(-gameConfig.tileSize);
        });

        test('wraps entity at mazeWidth + tileSize boundary', () => {
            const mazeWidth = gameConfig.mazeWidth * gameConfig.tileSize;
            const entity = {
                x: mazeWidth + gameConfig.tileSize
            };
        ...
        test('wraps entity beyond mazeWidth + tileSize', () => {
            const mazeWidth = gameConfig.mazeWidth * gameConfig.tileSize;
            const entity = {
                x: mazeWidth + gameConfig.tileSize * 2
            };
    ...
    describe('Entity within bounds', () => {
        test('does not modify entity within bounds (left side)', () => {
            const entity = {
                x: 0
            };
        ...
        test('does not modify entity within bounds (right side)', () => {
            const mazeWidth = gameConfig.mazeWidth * gameConfig.tileSize;
            const entity = {
                x: mazeWidth
            };
        ...
        test('does not modify entity in middle of maze', () => {
            const entity = {
                x: gameConfig.mazeWidth * gameConfig.tileSize / 2
            };
        ...
        test('does not modify entity at positive boundary', () => {
            const entity = {
                x: -gameConfig.tileSize + 1
            };
        ...
        test('does not modify entity at negative boundary', () => {
            const mazeWidth = gameConfig.mazeWidth * gameConfig.tileSize;
            const entity = {
                x: mazeWidth + gameConfig.tileSize - 1
            };
    ...
    describe('Boundary edge cases', () => {
        test('handles entity exactly at -tileSize', () => {
            const mazeWidth = gameConfig.mazeWidth * gameConfig.tileSize;
            const entity = {
                x: -gameConfig.tileSize
            };
        ...
        test('handles entity exactly at mazeWidth + tileSize', () => {
            const mazeWidth = gameConfig.mazeWidth * gameConfig.tileSize;
            const entity = {
                x: mazeWidth + gameConfig.tileSize
            };
    ...
    describe('Integration with gameConfig', () => {
        test('uses gameConfig values for calculations', () => {
            const mazeWidth = gameConfig.mazeWidth * gameConfig.tileSize;
            const tileSize = gameConfig.tileSize;

            const entityLeft = { x: -tileSize };
            const entityRight = { x: mazeWidth + tileSize };

            handleTunnelWrap(entityLeft);
            handleTunnelWrap(entityRight);

            expect(entityLeft.x).toBe(mazeWidth + tileSize);
            expect(entityRight.x).toBe(-tileSize);
        });
    });
});
